...
| An item breaks where it no longer fits | A claim was measured for its fence but written beside its label, uncounted — 14 item lines ran past 80, the longest 113. A claim that no longer fits beside its label now takes the line under it, the break saying what the dash said — a hard break, since a soft one is reflowed away and puts back the line it was breaking. Document-only. |
| The document is 90 columns wide | A test's specification is read in source beside the code that produced it and stays at 80; the document is read on its own and looked thin. Widening it put 8 items back onto one line. Document-only. |
