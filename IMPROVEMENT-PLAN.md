...
- [x] Complete 2026-07-25, except the opportunistic naming pass below.
...
- [x] Done 2026-07-25 — `ObjectStrategy`'s silent fallback, solved by a **general setup-warning mechanism**, not by anything generation-specific. `IAssertSpecificationContext.AddSetupWarning(string)` collects plain text (deduped, insertion-ordered, gathered for the whole test); `SpecificationContext` renders it under `=== WARNINGS ===` when — and only when — an assertion fails. `ObjectStrategy` is the first producer and the only place that knows what the warning means.
  - **The design changed twice under discussion.** First idea (per the original plan) was to record the warning into the specification text: rejected, because every spec touching such a type would carry the warning forever, including passing ones. Second idea was to delete the fallback and fail hard: rejected by the user's counter-example — a class whose parameterless constructor legitimately delegates to another with defaults is indistinguishable, at the generation site, from one that leaves an invariant broken. The difference is semantic, so the answer is to surface the fact and let the reader judge. That also means **no behavior change**: the fallback still happens.
  - Output shape: message, specification, `=== WARNINGS ===`, `=== VALUES ===`; sections omitted when empty, warnings separated by a blank line so multi-line ones stay distinguishable. The values header replaced the old `----`, which collided visually with xUnit's own inner-exception separator.
  - Tests: `WhenRenderFailureOutput` (14) covers the full matrix — {passing, failing} × {nested assertion failure or not} × {warning or not} × {values or not} — plus multi-line warnings and dedup. This was previously untested: the 174 failures the change first produced all went through the `HasMessage`/`HasAssignments` helpers, so adjusting those absorbed the format change without a single expectation being reviewed.
  - The hard-failure message now names the remedy (`Using<T>(...)` / `Given<T>(...)`), closing the cheap alternative left over from dropped P12.
...
- [x] Done 2026-07-25 — all three `For.None` questions closed by **one guard at the public boundary** rather than three separate decisions. The four scope-taking overloads in `Spec_Using` call `ValidateScope`, which rejects `For.None` with a `SetupFailed` naming the valid scopes.
  - **`For.None` is the empty scope, never a valid argument.** It has to keep existing — `TypeConversionStrategy.Register` relies on it as the result of intersecting disjoint scopes (`(existing & scope) != For.None`) — but nothing should ever pass it in. The XML docs previously advertised it as a choice ("or neither (`None`)", "The value applies neither to Input nor Subject"); they now say it is not accepted, so users aren't punished for believing the documentation.
  - Before: `Using(5, For.None)` gave `SetupFailed: Unsupported scope: None`, while `Using<int>(For.None).From<byte>()` gave **`NotImplementedException: None`** — the framework blaming itself for a user's typo, because `Register`'s overlap guard cannot fire when the scope *is* `None`. Both now give the same clear `SetupFailed`.
  - **This is why the boundary guard beats patching `Register`:** with the edge validated, the deep `_ =>` arms in `DataProvider` are genuinely unreachable by users, so they now throw `NotImplementedException` — which restores the principle exactly (user setup error → `SetupFailed` at the boundary; a `NotImplementedException` deeper in means a real framework gap).
  - Tests: `WhenUsingScopeNone` (4) covers value, factory, tag and conversion overloads. Suite 1344 green on net8/9/10.
