...
public class WhenRenderDocument : Spec
{
    ...
    /// <summary>
    /// What the file says of itself stands directly above the first section: a title is the only
    /// level-one heading in the file, so the sections below it are its own and need no rule to set
    /// them off.
    /// </summary>
    [Fact]
    public void ThenWriteWhatTheFileSaysOfItselfAboveTheFirstSection()
    ...
    public void ThenStateTheSubjectVersion()
        => Render(_respondOk).Does().Contain("Version 0.1.0, generated by TSpec");

    /// <summary>
    /// A project named for a layer is dotted — "MyHotel.Core" — and the file for what sits at its
    /// root is named as the project is, by the last part alone. The comment names the spec assembly
    /// in full, so the file still says which project it was generated from.
    /// </summary>
    [Fact]
    public void GivenADottedSubjectName_ThenNameTheRootFileByItsLastPart()
    {
        ...
        file.Content.Does().StartWith(
            "# Core\n\n<!-- Version 0.1.0, generated by TSpec from MyHotel.Core.Spec. Do not edit by hand. -->\n");
    }
    ...
    // ----------- The top-level folder is the file, and what sits at the root is a file of its own
    ...
    /// so the segment below the project's own is the file, titled by that segment.
    /// </summary>
    [Fact]
    public void GivenTheNamespacesDiffer_ThenWriteOneFilePerTopLevelFolder()
    {
        var files = Files(
            InNamespace("MyHotel.Spec.Rooms", "WhenAddRoom", "ThenA"),
            InNamespace("MyHotel.Spec.Bookings", "WhenBookRoom", "ThenB"));
        Names(files).Is().EqualTo(["Bookings", "Rooms"]);
        Named(files, "Bookings").Does().StartWith("# Bookings\n\n<!-- ").and.Contain("-->\n\n## When book room\n");
        Named(files, "Rooms").Does().StartWith("# Rooms\n\n<!-- ").and.Contain("-->\n\n## When add room\n");
    }

    /// <summary>
    /// The comment says where the file was generated from, and for a folder's file that is the
    /// folder: the assembly alone would name the project, which the root file is generated from.
    /// </summary>
    [Fact]
    public void GivenAFoldersFile_ThenNameTheFolderAsWhatItWasGeneratedFrom()
    {
        var files = Files(
            InNamespace("MyHotel.Spec", "WhenGetVersion", "ThenA"),
            InNamespace("MyHotel.Spec.Rooms", "WhenAddRoom", "ThenB"));
        Named(files, "Rooms").Does().Contain("generated by TSpec from MyHotel.Spec/Rooms. Do not edit by hand.");
        Named(files, "MyHotel").Does().Contain("generated by TSpec from MyHotel.Spec. Do not edit by hand.");
    }

    /// <summary>
    /// A folder is a file whether or not there is another beside it: the split follows the project's
    /// layout, and a project with one folder has one file named for it.
    /// </summary>
    [Fact]
    public void GivenEverySpecSitsInOneFolder_ThenStillNameTheFileForIt()
        => Names(Files(
            InNamespace("MyHotel.Spec.Rooms", "WhenAddRoom", "ThenA"),
    ...
    /// <summary>
    /// A spec at the root belongs to no folder, so it goes to the file named for the project — and
    /// only there, the folders' files holding nothing but what their folders hold.
    /// </summary>
    [Fact]
    public void GivenASpecSitsAtTheRoot_ThenWriteItToTheProjectsOwnFile()
    {
        var files = Files(
            InNamespace("MyHotel.Spec", "WhenGetVersion", "ThenA"),
            InNamespace("MyHotel.Spec.Rooms", "WhenAddRoom", "ThenB"));
        Names(files).Is().EqualTo(["MyHotel", "Rooms"]);
        Named(files, "MyHotel").Does().StartWith("# My Hotel\n\n<!-- ")
            .and.Contain("-->\n\n## When get version\n\n- **a** — `a`\n")
            .and.not.Contain("Rooms");
    }

    /// <summary>
    /// A folder named as the project is would be written to the file the root already takes, and
    /// one file cannot hold both — so the run fails naming the clash rather than writing one over
    /// the other.
    /// </summary>
    [Fact]
    public void GivenAFolderIsNamedAsTheProject_ThenFail()
        => Xunit.Assert.Throws<SetupFailed>(() => Files(
            InNamespace("MyHotel.Spec", "WhenGetVersion", "ThenA"),
            InNamespace("MyHotel.Spec.MyHotel", "WhenAddRoom", "ThenB")))
            .Message.Does().Contain("MyHotel");

    /// <summary>
    /// One class under test per folder leaves the file's own title saying everything there is to
    /// say, so what runs below it is not written: a file per folder and a heading below its title
    /// naming the only thing in it would be the same division twice.
    /// </summary>
    [Fact]
    public void GivenTheNamespacesRunDeeper_ThenHeadOnlyTheSegmentTheyDifferIn()
    {
        var files = DocumentRenderer.Render(new("MyHotel.Core", "0.1.0"), "MyHotel.Core.Spec", [
        ...
        Names(files).Is().EqualTo(["Bookings", "Rooms"]);
        Named(files, "Rooms").Does().Contain("-->\n\n## When add room\n").and.not.Contain("Service");
    }
    ...
    /// <summary>
    /// Where a folder holds more than one namespace below it, each heads a group of its own: the
    /// file names what they share and the group what tells them apart. Everything left below the
    /// folder merges into one name, the dots separating parts of it rather than sentences.
    /// </summary>
    [Fact]
    public void GivenAFolderHoldsSeveralNamespaces_ThenGroupThemBelowItsTitle()
    {
        var files = Files(
        ...
        Named(files, "A").Does().Contain("-->\n\n## B C\n").and.Contain("\n## B D\n");
        Named(files, "E").Does().Contain("-->\n\n## F G\n").and.Contain("\n## H I\n");
    }

    /// <summary>
    /// A group heading pushes what is under it a level down, so a subject sits below the group that
    /// holds it — and stays where it was in a file that never grouped, rather than paying a level
    /// for a division that was not made.
    /// </summary>
    [Fact]
    public void GivenOnlyOneFolderGroups_ThenLeaveTheOtherAtItsOwnDepth()
    {
        var files = Files(
        ...
        Named(files, "Rooms").Does().Contain("-->\n\n## When add room\n");
        Named(files, "Bookings").Does().Contain("-->\n\n## Booking Number Generator\n\n### When next\n");
    }
    ...
    /// </summary>
    [Fact]
    public void GivenEverySpecOfAFolderDeclaresTheSame_ThenStateItAtTheTopOfItsFile()
    {
        var files = Files(
            InNamespace("MyHotel.Spec.Rooms", "WhenAddRoom", "ThenA")
            ...
            InNamespace("MyHotel.Spec.Rooms", "WhenGetRoom", "ThenB")
                with { SubjectUnderTest = "RoomService" },
            InNamespace("MyHotel.Spec.Bookings", "WhenBook", "ThenC")
                with { SubjectUnderTest = "BookingService" });
        Named(files, "Bookings").Does().Contain("-->\n`Subject under test: BookingService`\n");
        Named(files, "Rooms").Does().Contain("-->\n`Subject under test: RoomService`\n");
    }

    /// <summary>A clause every requirement of a folder states settles at the top of its file.</summary>
    [Fact]
    public void GivenEveryRequirementOfAFolderOpensAlike_ThenStateThatOnceForTheFile()
        => Named(Files(
            ...
            InNamespace("A.Bookings", "WhenBook", "ThenC")), "Rooms")
            .Does().Contain("-->\n`Having post`\n").and.Contain("- **a** — `a`\n");

    /// <summary>
    /// Files are listed by name: a folder's file is found by its name, and there is no reading order
    /// across files for complexity to decide.
    /// </summary>
    [Fact]
    public void ThenListTheFilesByName()
        => Names(Files(
            InNamespace("A.Zzz", "WhenB", "ThenB"),
            InNamespace("A.Aaa", "WhenA", "ThenA") with { Clauses = [Condition("post"), Claim("then a")] }))
            .Is().EqualTo(["Aaa", "Zzz"]);

    /// <summary>Two classes of one name in different folders are two requirements, not a duplicate.</summary>
    [Fact]
    public void GivenTwoFoldersHoldASubjectOfTheSameName_ThenRenderBoth()
        => Names(Files(
