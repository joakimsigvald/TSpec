...
- Any clause every requirement below a heading states is written once at that heading instead of repeated in each block, wherever it sits in the specification — so a branch's own `Given` does not keep the subject's shared `When` out of the heading named after it. A clause rises as many times as the requirement stating it fewest times does, since two identical setups are two invocations. This also covers — including `Subject under test:` and `Return type:` from the spec's `Spec<TSUT, TResult>`. Those two hoist **independently**, each to the highest heading where every requirement below agrees on it, so one subject can head a whole document while each section states its own return type. Assertions never move up.
