...
Every TSpec test carries a readable specification of what it claims. Structure the suite so those
read well, and TSpec can collect them into a single `SPECIFICATION.md` describing the system —
...
### 6.2 Generating SPECIFICATION.md
...
`SPECIFICATION.md` is then written to the spec project root when the run ends. Without that line
nothing is collected and nothing changes.

> **Work in progress.** There is no way yet to keep an individual spec out of the document.
...
The structure is your test structure: the folder becomes a section, the `When…` class a subject, each
`Given…` class a branch, and each `Then…` method an item. `MyHotel.Spec/Rooms/WhenAddRoom.cs` —
...
— becomes this, rendered as you see it:

> # My Hotel
>
> <!-- Version 0.1.0+764a903d, generated by TSpec from MyHotel.Spec. Do not edit by hand. -->
> ```
...
Three things follow from that, and are worth writing tests for:

- **Names are read as prose.** `ThenRespondCreated` becomes **respond created**. Name a test method
  after the claim it makes, not after the mechanism it uses.
- **Arrangement shared by a heading's requirements is stated once, at that heading.** `When add room`
  appears once above because both `Given` classes inherit it — so put in the outer class whatever
  holds for everything below it.
- **`Spec<TSUT, TResult>` is what states the types.** They are written as `Subject under test:` and
  `Return type:` at the highest heading where everything below agrees on them.

#### Keeping it fresh

The document is deterministic — sorted, deduplicated, LF-normalized — so the same source produces a
byte-identical file on every machine, and a stale one shows up as a diff. Let the build catch it:
