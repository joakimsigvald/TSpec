...
heading rather than repeated in each block — above, the subject and return type and the two `Using`
clauses hold for the whole document, and the `When` for the whole subject. Assertions never move up:
...
it has already said: a `When` heading is not followed by `When`, and a list item does not spell out
the `Then` its place in the list already states.
