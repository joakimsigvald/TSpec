...
7. [Generating SPECIFICATION.md](#7-generating-specificationmd)
...
are declared does not matter. Because execution is deferred until assertion, TSpec can deterministically reorder the pipeline before running it.
...
* **Smart Casting:** Once mapped, TSpec automatically probes the requested type for compatibility. It attempts to construct the target by finding implicit cast operators, single-parameter constructors, or matching static factory methods (e.g., `Create()`).
...
This chapter assumes familiarity with Moq or a similar mocking framework, and shows how TSpec simplifies the mocking experience.
...
This is an opinionated recommendation for structuring your tests, based on how xUnit and TSpec work and on experience from real projects.
...
TSpec is designed to thrive in clean, well-structured codebases,
...
## 7. Generating SPECIFICATION.md

Every TSpec test already carries a readable specification. TSpec can collect them into a single
`SPECIFICATION.md` at the root of the spec project — a document describing the system under test,
regenerated by running the tests and reviewable in a pull request like any other file.

> **Work in progress.** Not yet done: opt-out attributes to keep a spec out of the document.

The document lists each passing requirement under its subject, with the specification exactly as a
failing test would report it:

````markdown
# My Hotel

<!-- Version 0.1.0+27e80a19, generated by TSpec from MyHotel.Spec. Do not edit by hand. -->
```
...
rather than about the section. That holds only where the phrase fits on the act's line: wrapped, it
would start a line with its own comma, so a long act gets `Return type:` as a label written under
it instead — still qualifying the act, and still said once. The two declared labels hoist independently, each to the
...
clauses that act on it — `When(_ => _.Api.GetAsync("/rooms"))` reads `When Api.GetAsync("/rooms")`,
because `Subject under test:` has named it already. That applies wherever the parameter heads a
...
One line in the spec project:

```csharp
[assembly: AssemblyFixture(typeof(SpecificationDocument))]
```

The document is written when the test run ends. A project without that line behaves exactly as
...
which project that is. It does not guess:

1. The subject name is the spec assembly name with its last suffix stripped — `MyHotel.Spec`
   describes `MyHotel`, `MyHotel.Api.Tests` describes `MyHotel.Api`. Any suffix works; the
   name is not matched against a list of known ones.
2. That name is then **verified** against the project references the build recorded for the spec
   assembly. The version in the document is the one the build resolved for that project, so it
   comes from `<Version>` in the production project file.

If either half fails — the name has no suffix to strip, or the derived name is not a direct
project reference — TSpec throws `SetupFailed` at the start of the run. The message states both
expectations, whichever half broke, and lists the references it did find:

```
TSpec derived the subject 'Foo' from the spec assembly name 'Foo.Spec', but 'Foo' is not one of
its direct project references (MyHotel, TSpec). A spec project must (1) be named after the
project it specifies with one suffix appended — 'MyHotel.Spec' is preferred, 'MyHotel.Test' is
fine — and (2) reference that project directly; a transitive reference is not enough.
```

This follows §6.1's recommended structure, so a project laid out as recommended needs no
configuration. Everything the document depends on is resolved before the first test runs, so a
misconfigured project fails immediately rather than after the suite has finished.
