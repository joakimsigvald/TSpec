...
> **Work in progress.** Every passing requirement is collected and rendered, grouped by the class
> under test. Not yet done: opt-out attributes, and the completeness check that would catch a
> filtered run (see below).
...
Entries are sorted by subject, branch and requirement, and identical entries collapse to one — so a
theory contributes a single line and parallel execution order never reaches the page.
