...
internal record DoesValue<TValue> : Constraint<TValue, DoesValue<TValue>>
{
    ...
    {
        Assert(conditionExpr!.Describe(), NotNullAnd(actual => Xunit.Assert.True(condition(actual))), conditionExpr!, "", verbalizationStrategy: VerbalizationStrategy.PresentSingularS);
        return new(Actual);
    }
}