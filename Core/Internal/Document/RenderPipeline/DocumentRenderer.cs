...
internal static class DocumentRenderer
{
    ...
    private static DocumentSegment[] Layout(this Document _document)
    {
        ...
        Add(new CodeSegment(
            _document.SubjectUnderTest, ReturnType: null, _document.Whole, Stated: null));
        ...
        void AddRequirements()
        {
            ...
            foreach (var area in Order(_document.Areas))
                AddNode(area, _document.SubjectUnderTest);
        }

        void AddNode(DocumentNode node, string? statedSubject)
        {
            ...
            if (node is BranchNode branch)
                ...
                foreach (var section in Order(node.Children))
                    AddNode(section, statedSubject ?? node.SubjectUnderTest);
        }
