...
{
    /// Below this many requirements, shared opening clauses are not repetition worth factoring out
    /// and the reader is better served by a block that stands alone.
    private const int SharedOpeningThreshold = 3;
        ...
        IEnumerable<SpecificationEntry> entries)
    {
        ...
        foreach (var group in requirements.Select(r => r.Without(whole.Count)).GroupBy(r => r.Entry.Subject))
        {
            ...
            foreach (var requirement in ofSubject.Select(r => r.Without(shared.Count)))
                text.Append($"\n### {Heading(requirement.Entry)}\n"
    ...
    /// <summary>
    /// The clauses every requirement here opens with — stated once for the group instead of
    /// repeated in each block. Whole clauses only, never a fragment of one.
    /// </summary>
    /// <remarks>
    ...
    {
        if (requirements.Length < SharedOpeningThreshold)
            return [];
