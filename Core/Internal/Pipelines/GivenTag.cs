    ...
    : IGivenTag<TSUT, TResult, TValue>
{
    public IGivenTestPipeline<TSUT, TResult> Is(
        ...
        => spec.Apply<TValue>(() => spec.Assign(tag, value, tagExpr), $"{tagExpr} is {valueExpr!.Describe()}", true);

    public IGivenTestPipeline<TSUT, TResult> Has(
        Action<TValue> setup,
        [CallerArgumentExpression(nameof(setup))] string? setupExpr = null)
        => spec.Apply<TValue>(() => spec.Apply(tag, setup, tagExpr), $"{tagExpr} has {setupExpr!.Describe()}", true);

    public IGivenTestPipeline<TSUT, TResult> Has(
        Func<TValue, TValue> transform,
        [CallerArgumentExpression(nameof(transform))] string? transformExpr = null)
        => spec.Apply<TValue>(() => spec.Apply(tag, transform, tagExpr), $"{tagExpr} has {transformExpr!.Describe()}", true);
}