...
internal class SpecFixture<TSUT>(ISpecificationProvider specificationProvider) : IDisposable
{
        ...
        Command command, [CallerArgumentExpression(nameof(command))] string? commandName = null)
    {
        return command.Invocation switch
        {
            ...
            Func<TSUT, ValueTask<TResult>> queryAsync => (AsyncHelper.Execute(() => queryAsync(SubjectUnderTest).AsTask()), true),
            Func<ValueTask<TResult>> queryAsync => (AsyncHelper.Execute(() => queryAsync().AsTask()), true),
            Func<TSUT, ValueTask> actAsync => ActOnSubjectAndReturnAsync(_ => actAsync(_).AsTask()),
            Func<ValueTask> actAsync => ActAndReturnAsync(() => actAsync().AsTask()),
